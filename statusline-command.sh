...
PART1=$(echo "$INPUT" | npx -y @owloops/claude-powerline@1.16.1 --theme=tokyo-night --style=capsule)
# Strip context_window to force transcript-based calculation (workaround for anthropics/claude-code#13783)
PART2=$(echo "$INPUT" | jq 'del(.context_window)' | npx -y ccusage@18.0.5 statusline --visual-burn-rate emoji)
